...
// Allow optional url so we can verify it if your backend returns it
export type ImageRec = { id: string; caption: string; url?: string };
...
export const PhotoUploader: React.FC<PhotoUploaderProps> = ({
  ...
  // Verify a blob URL actually resolves (only if ImageRec.url is present)
  const verifyUrl = (url: string, timeoutMs = 7000): Promise<void> =>
    new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        clearTimeout(timer);
        reject(new Error("Blob URL not reachable"));
      };
  ...
  const uploadAll = async (): Promise<ImageRec[]> => {
    ...
    // Only attempt valid candidates
    const candidates = items.filter((i) => i.status !== "error");
    ...
    for (const it of candidates) {
      ...
      try {
        ...
        // If a URL is returned, verify it loads before marking done
        if (image?.url && typeof image.url === "string") {
          await verifyUrl(image.url);
        }
    ...
    <div className="w-[80%]">
      ...
      {onUpload && (
        <div className="mt-3 flex items-center justify-end gap-2">
          <button
            type="button"
            disabled={busy || disabled || validCount === 0}
            onClick={uploadAll}
            className={`rounded-xl px-4 py-2 text-white ${
              busy || disabled || validCount === 0
                ? "bg-gray-400"
                : "bg-green-600 hover:bg-green-700"
            }`}
          >
