...
export type PhotoUploaderProps = {
  ...
  maxSizeMB?: number; // per-file size check (default 10)
  disabled?: boolean;

  // LOCAL MODE: get selected Files (no uploading)
  onFilesChange?: (files: File[]) => void;

  // UPLOAD MODE (optional): called for each file; return your Image record
  // Example impl (Firebase/S3/etc) should resolve URL/ID and call onProgress(0..100)
  onUpload?: (file: File, onProgress: (p: number) => void) => Promise<ImageRec>;
  autoUpload?: boolean; // if true, start uploading immediately after select

  // Called after all uploads finish (only if onUpload is provided)
  onUploadComplete?: (images: ImageRec[]) => void;

  // Start uploads manually (call the function you get via this ref)
