    ...
    <div className="px-6 pb-6">
      {error && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl">
          ...
          <button onClick={loadHobbies} className="px-3 py-2 bg-red-600 text-white rounded-md text-sm">
            Try Again
          </button>
        </div>
      )}
      ...
      <div className="mx-auto w-[380px] rounded-xl border border-[#E6E6E6] shadow-[0_2px_6px_rgba(0,0,0,0.06)] bg-white overflow-hidden">
        <div className="max-h-[260px] overflow-y-auto divide-y">
          {hobbies.map((hobby) => (
            <div key={hobby.id} className="flex items-center justify-between px-4 py-3 bg-white">
              {editingId === hobby.id ? (
                <input
                  type="text"
                  value={editingName}
              ...
              <div className="flex items-center gap-2">
                {editingId === hobby.id ? (
                  <>
                    <button onClick={() => handleUpdateHobby(hobby.id)} className="px-3 py-1 bg-green-600 text-white rounded-md text-xs">
                      Save
                    </button>
                    <button onClick={cancelEditing} className="px-3 py-1 bg-gray-500 text-white rounded-md text-xs">
                      Cancel
                    </button>
                  </>
                ) : (
                  <>
                    <EditIcon className="cursor-pointer text-orange-600" onClick={() => startEditing(hobby)} />
                    <DeleteIcon className="cursor-pointer text-orange-600" onClick={() => handleDeleteHobby(hobby.id)} />
                  </>
                )}
              </div>
            </div>
          ...
          <div className="px-4 py-3 bg-white">
            {!isCreating ? (
              <button onClick={() => setIsCreating(true)} className="text-sm text-[#5B5B5B] hover:underline">
                Add Option...
              </button>
            ) : (
              <form onSubmit={handleCreateHobby} className="flex items-center gap-2">
                <input
                  type="text"
                  value={newHobbyName}
                  ...
                  autoFocus
                />
                <button type="submit" disabled={!newHobbyName.trim()} className="px-3 py-2 bg-green-600 text-white rounded-md text-xs disabled:opacity-50">
                  Save
                </button>
                <button
                  type="button"
    ...
    return (
      <div className="relative bg-[#0A2A33] min-h-screen w-full flex items-start justify-center">
        <section
          ...
          style={{ position: 'absolute', top: 184, left: '50%', transform: 'translateX(-50%)', width: 720, height: 1074 }}
        >
          <div className="flex items-center justify-center h-full text-gray-500">Loading quiet hobbies...</div>
        </section>
      </div>
    );
  }
    ...
    <div className="relative bg-[#0A2A33] min-h-screen w-full flex items-start justify-center">
      ...
      >
        ...
        <header className="relative px-6 py-5">
          <button type="button" aria-label="Go back" className="absolute left-6 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-[#FFD2E4] text-[#FF2680] grid place-items-center">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="-ml-[1px]"><path d="M15 6l-6 6 6 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
          </button>
          <h2 className="text-center text-[22px] font-semibold text-[#0B3A3F]">Create Event</h2>
          <button type="button" aria-label="Close" className="absolute right-6 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-[#FFD2E4] text-[#FF2680] grid place-items-center">
        ...
        <div className="px-6 pt-1">
          <div className="w-[220px] mx-auto bg-[#EDEDED] rounded-full p-1 flex">
            <button type="button" className="flex-1 px-4 py-2 rounded-full text-[#A2A2A2] cursor-not-allowed">Task</button>
            <button type="button" aria-current="page" className="flex-1 px-4 py-2 rounded-full bg-[#FD8743] text-white font-medium shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)]">Choice</button>
          </div>
        </div>
        ...
        <footer className="px-6 pb-8 grid place-items-center">
          <button type="button" disabled className="w-[360px] h-12 rounded-xl font-semibold shadow-sm bg-[#E3E3E3] text-[#B5B5B5] cursor-not-allowed">Finish</button>
        </footer>
      </section>
    </div>
  );
