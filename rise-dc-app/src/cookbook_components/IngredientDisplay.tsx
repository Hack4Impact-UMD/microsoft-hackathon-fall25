import React, { useState} from "react";
import ImageCard from "../shared/components/ImageCard";
import Button from "../cookbook_components/Button";
import { RecipeIngredient } from "../shared/types";

interface IngredientDisplayProps {
    src: string;
    ingredient: RecipeIngredient;
    height?: number;
    width?: number;
    alt?: string;
    className?: string;
    onToggle?: (ingredient: RecipeIngredient, hasIngredient: boolean) => void;
}

const IngredientDisplay: React.FC<IngredientDisplayProps> = ({ src, ingredient, height, width, alt, className, onToggle }) => {
    const [hasIngredient, setHasIngredient] = useState(false);

    const handleToggle = () => {
        const currState = !hasIngredient;
        setHasIngredient(currState);
        onToggle?.(ingredient, currState);
    };

    return (
   <div className={`flex flex-col border-2 rounded-lg items-center w-fit relative ${className}`}>
      <button
        onClick={handleToggle}
        className={`absolute top-1 right-1 w-6 h-6 sm:w-8 sm:h-8 rounded border border-gray-800 flex items-center justify-center transition-colors duration-200 z-10 ${
          hasIngredient 
            ? 'bg-gray-800 text-white' 
            : 'bg-white text-transparent hover:bg-gray-100'
        }`}
        aria-label={`Toggle ${ingredient} availability`}
      >
        {hasIngredient && (
          <svg 
            className="w-3 h-3 sm:w-5 sm:h-5" 
            fill="none" 
            stroke="currentColor" 
            viewBox="0 0 24 24"
          >
            <path 
              strokeLinecap="round" 
              strokeLinejoin="round" 
              strokeWidth={2} 
              d="M5 13l4 4L19 7" 
            />
          </svg>
        )}
      </button>

      <img 
        src={src} 
        alt={alt || `${ingredient} ingredient`} 
        style={height || width ? { height, width } : undefined}
        className={`object-cover rounded-t-lg ${height || width ? '' : 'w-full'}`} 
      />
      <div className="w-full text-lg text-center py-5 font-bold rounded-b-lg">
        {ingredient.quantity} {ingredient.ingredient.name}
      </div>
    </div>
  );
};

export default IngredientDisplay;
    
