...
export type AssignedMap = Record<string, Partial<Record<MealType, { id: string; title: string; image?: string }>>>
...
interface MealPlanGridProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  onOpen?: (info: { dateISO: string; mealType: MealType; recipe: { id: string; title: string; image?: string } }) => void
}

function getStartOfWeek(anchor: Date, weekStartsOn: 'monday' | 'sunday') {
  const m = moment(anchor)
  return weekStartsOn === 'monday' ? m.clone().startOf('week').add(1, 'day') : m.clone().startOf('week')
}

const DEFAULT_MEAL_TYPES: MealType[] = ['breakfast', 'lunch', 'dinner', 'snack', 'dessert']
...
export default function MealPlanGrid({
  ...
  const [anchorDate, setAnchorDate] = React.useState<Date>(startDate ?? new Date())

  const startOfWeek = React.useMemo(() => getStartOfWeek(anchorDate, weekStartsOn), [anchorDate, weekStartsOn])
  const days = React.useMemo(() => Array.from({ length: 7 }, (_, i) => startOfWeek.clone().add(i, 'day')), [startOfWeek])
  ...
  const scaleWidth = (cardInnerWidth - rowHeaderWidth - gapPx * 7) / (7 * baseCellW)
  const scaleHeight = (gridAvailableHeight - columnHeaderHeight - gapPx * 5) / (5 * baseCellH)
  const scale = Math.max(0.6, Math.min(1, Math.min(scaleWidth, scaleHeight)))
  const dayColWidth = baseCellW * scale

  return (
    <div className={`p-8 w-[704px] h-[804px] rounded-[20px] bg-white ${className}`} {...rest}>
      {/* Top month header with week navigation */}
      <div className="w-full h-[56px] flex items-center justify-center gap-3 mb-4">
        <button aria-label="Previous week" onClick={() => changeWeek(-7)} className="p-1 rounded hover:bg-gray-100">
          <ChevronLeftIcon />
        </button>
        <h2 className="text-2xl font-semibold">{monthTitle}</h2>
        <button aria-label="Next week" onClick={() => changeWeek(7)} className="p-1 rounded hover:bg-gray-100">
          <ChevronRightIcon />
        </button>
      </div>
      ...
      <div>
        <div className="inline-block">
          <div
            className="grid"
            style={{ gridTemplateColumns: `${rowHeaderWidth}px repeat(7, ${dayColWidth}px)`, gap: `${gapPx}px` }}
          >
            {/* Column headers row */}
            <div />
            {days.map((d) => (
              ...
              >
                <div className="text-sm text-gray-700 leading-tight">{d.format('ddd')}</div>
                <div className="text-base font-medium leading-tight">{d.format('D')}</div>
              </div>
            ))}
              ...
              <React.Fragment key={`row-${mt}`}>
                ...
                >
                  <div className="text-2xl mb-2">
                    {mt === 'breakfast' ? '🍳' : mt === 'lunch' ? '🥪' : mt === 'dinner' ? '🍽️' : mt === 'snack' ? '🥨' : '🧁'}
                  </div>
                  <div className="text-sm font-medium text-gray-700 capitalize">{mt}</div>
                </div>
                ...
                {days.map((d) => {
                  ...
                  return (
                    <div key={`cell-${mt}-${iso}`} className="flex items-center justify-center">
                      {/* Scaled wrapper so the fixed-size MealPlanCell fits layout width */}
                      <div
                        style={{ width: `${baseCellW * scale}px`, height: `${baseCellH * scale}px` }}
                      >
                        <div style={{ transform: `scale(${scale})`, transformOrigin: 'top left' }}>
                          <MealPlanCell
                            mealType={mt}
                            dateLabel={`${d.format('ddd')} ${d.format('D')}`}
                            recipe={recipe}
                            onAdd={() => onAdd?.({ dateISO: iso, mealType: mt })}
                            onOpen={(r) => onOpen?.({ dateISO: iso, mealType: mt, recipe: r })}
                          />
                        </div>
                      </div>
                    </div>
