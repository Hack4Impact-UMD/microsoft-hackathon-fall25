...
cookbookRouter.post("/recipes", async (req: Request, res: Response) => {
  try {
    const recipe: Recipe = {
      ...req.body,
      id: req.body.id || `recipe_${crypto.randomUUID()}`,
    };
...
cookbookRouter.put("/recipes/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (id !== recipe.id) {
      return res
        .status(400)
        .json({ error: "ID in URL does not match ID in body" });
    }
...
cookbookRouter.post("/ingredients", async (req: Request, res: Response) => {
  try {
    const ingredient: Ingredient = {
      ...req.body,
      id: req.body.id || `ingredient_${crypto.randomUUID()}`,
    };
...
cookbookRouter.put("/ingredients/:id", async (req: Request, res: Response) => {
  try {
    ...
    const { resource: updatedIngredient } =
      await container.items.upsert(ingredient);
...
cookbookRouter.post("/tools", async (req: Request, res: Response) => {
  try {
    const tool: Tool = {
      ...req.body,
      id: req.body.id || `tool_${crypto.randomUUID()}`,
    };
...
cookbookRouter.post("/utensils", async (req: Request, res: Response) => {
  try {
    const utensil: Utensil = {
      ...req.body,
      id: req.body.id || `utensil_${crypto.randomUUID()}`,
    };
...
cookbookRouter.post("/mealpreps", async (req: Request, res: Response) => {
  try {
    const mealPrep: MealPrep = {
      ...req.body,
      id: req.body.id || `mealprep_${crypto.randomUUID()}`,
    };
...
cookbookRouter.put("/mealpreps/:id", async (req: Request, res: Response) => {
  try {
    ...
    const { resource: updatedMealPrep } =
      await container.items.upsert(mealPrep);
...
cookbookRouter.post("/grocerylists", async (req: Request, res: Response) => {
  try {
    const groceryList: GroceryList = {
      ...req.body,
      id: req.body.id || `grocerylist_${crypto.randomUUID()}`,
    };
...
cookbookRouter.put("/grocerylists/:id", async (req: Request, res: Response) => {
  try {
    ...
    const { resource: updatedGroceryList } =
      await container.items.upsert(groceryList);
